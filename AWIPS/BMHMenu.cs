...
namespace AWIPS
{
    public partial class BMHMenu : Form
    {
        ...
        private string listDumpLast = "";
        private string listDumpLast2 = "";
        public BMHMenu()
        {
            InitializeComponent();
            ...
            btnTransmitterDetails.Enabled = false;
                    ...
                    using (JsonDocument doc = JsonDocument.Parse(jsonString))
                    {
                            ...
                            statusElement.GetString() == "success")
                        {
                            if (root.TryGetProperty("data", out JsonElement statusElement2))
                            {
                                ...
                                listDumpLast2 = "";
                                for (int i = 17; i < dataList2.Count; i += 1)
                                {
                                    listDumpLast2 += dataList2[i];
                                }
                                if (listDumpLast2 != listDumpLast)
                                {
                                    ...
                                    btnTransmitterDetails.Enabled = true;
                                    ...
                                    dgvTransmitter.Rows.Clear();
                                    for (int i = 17; i < dataList2.Count; i += 15)
                                    {
                                    ...
                                    if (selectedRowIndex >= 0 && selectedRowIndex < dgvTransmitter.Rows.Count)
                                    {
                                        ...
                                        tbMessageText.Text = dataList2[(selectedRowIndex * 15) + 31];
                                    }
                                    this.dgvTransmitter.ResumeLayout();
                                    listDumpLast = "";
                                    for (int i = 17; i < dataList2.Count; i += 1)
                                    {
                                        listDumpLast += dataList2[i];
                                    }
                                }
                                lastDataPull = input;
                            }
        ...
        private void dataGridView1_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            tbMessageText.Text = dataList2[(rowIndex * 15) + 31];
        }

        private void button1_Click_1(object sender, EventArgs e)
        {
            int rowIndex = dgvTransmitter.CurrentRow.Index;
            MessageBox.Show($"Transmit Time: {dataList2[(rowIndex * 15) + 8 + 12]}{Environment.NewLine}Expiration Time: {dataList2[(rowIndex * 15) + 8 + 18]}{Environment.NewLine}{Environment.NewLine}Message Id: {dataList2[(rowIndex * 15) + 8 + 14]}{Environment.NewLine}Message Title: {dataList2[(rowIndex * 15) + 8 + 15]}{Environment.NewLine}Message Name: {dataList2[(rowIndex * 15) + 8 + 16]}{Environment.NewLine}MRD: {dataList2[(rowIndex * 15) + 8 + 17]}{Environment.NewLine}Play Count: {dataList2[(rowIndex * 15) + 8 + 21]}{Environment.NewLine}{Environment.NewLine}SAME Header: {dataList2[(rowIndex * 15) + 8 + 22]}", "Message Details");
        }
        ...
        private void gbTransmitters_Enter(object sender, EventArgs e)
        {

        }

        private void btnTransmitterDetails_Click(object sender, EventArgs e)
        {
            MessageBox.Show($"Transmitter: {dataList2[0]}{Environment.NewLine}Frequency: {dataList2[14]}{Environment.NewLine}Location: {dataList2[8]}{Environment.NewLine}{Environment.NewLine}Current Time (UTC): {dataList2[15]}{Environment.NewLine}Est. Up-Time: {dataList2[16]}{Environment.NewLine}{Environment.NewLine}Current Startup Time (UTC): {dataList2[9]}{Environment.NewLine}Current Up-Time: {dataList2[11]}{Environment.NewLine}{Environment.NewLine}Lifetime First Startup (UTC): {dataList2[10]}{Environment.NewLine}Lifetime Up-Time: {dataList2[12]}{Environment.NewLine}{Environment.NewLine}Power Cycles: {dataList2[13]}", "Transmitter Details");
        }
    }
}